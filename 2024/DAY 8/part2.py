...
for freq in antennas:
    ...
    for i in range(0, len(coords)):
        for j in range(i + 1, len(coords)):
            ...
            curr = 0
            new_anticode = (coords[i][0] + displacement[0] * curr, coords[i][1] + displacement[1] * curr)

            # ensure that the coords is within range
            while new_anticode[0] >= 0 and new_anticode[0] < MAX_ROW and new_anticode[1] >= 0 and new_anticode[1] < MAX_COL:
                antinodes.add(new_anticode)
                curr += 1
                new_anticode = (coords[i][0] + displacement[0] * curr, coords[i][1] + displacement[1] * curr)
            
            curr = 0
            new_anticode = (coords[j][0] - displacement[0] * curr, coords[j][1] - displacement[1] * curr)
